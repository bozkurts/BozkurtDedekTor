...
import urlparse
from bozkurtrenk import Style,Fore,Back,init 
init()
if os.name == 'nt':
	os.system('cls')
else:
...
def bozkurt():
	print(" ____           _               _     ")
	print("| __ )  ___ ___| | ___   _ _ __| |_   ")
	print("|  _ \ / _ \_  / |/ / | | | '__| __|  ")
	print("| |_) | (_) / /|   <| |_| | |  | |_   ")
	print("|____/ \___/___|_|\_\\__,_|_|   \__|  ")
	print(" ____           _      _    _____     ") 
	print("|  _ \  ___  __| | ___| | _|_   _|__  _ __ ")
	print("| | | |/ _ \/ _` |/ _ \ |/ / | |/ _ \| '__|")
	print("| |_| |  __/ (_| |  __/   <  | | (_) | |   ")
	print("|____/ \___|\__,_|\___|_|\_\ |_|\___/|_|   by ankongeneral")
	def menu():
		global filedetector,wpenumerator,subdomainscan,portscan,wpscan,xssscanner,wpbackupscan,sqliscanner
		print(" -- "+boldwhite+"Menu"+reset+" -- \n \n  1. 	"+boldwhite+"WordPress Kullanici Adi Bulucu"+reset+"   \n  2. 	"+boldwhite+"Hassas Dosya Dedektoru"+reset+"        \n  3. 	"+boldwhite+"Alt Etki Alani Tarayici"+reset+"\n  4. 	"+boldwhite+"Port Tarayici"+reset+"        \n  5. 	"+boldwhite+"Wordpress Tarayici\n"+reset+"  6. 	"+boldwhite+"Siteler Arasi Komut Dosyasi [XSS] Tarayici\n"+reset+"  7.    "+boldwhite+"Wordpress Yedekleme Kapmak\n"+reset+"  8.    "+boldwhite+"SQL Enjeksiyon [SQLI] Tarayici\n"+reset)
		option = raw_input("[root@bozkurt] Secenegi Sec\n    > ")
		if option == "1":
			wpenumerator = "on"
		elif option == "2":
			filedetector = "on"
			...
			print("[root@bozkurt] Yanlis Secenek Secildi")
			menu()
	...
	def cloudflare():
		...
		if "Erisimi Kisitlamak Icin CloudFlare Kullaniliyor" in pagesource:
			print("[root@bozkurt] Cloudflare IP'yi engelledi")
			again()
	def alive():
		try:
			...
			print("[root@bozkurt]"+green+"Site acildi!"+reset)
			print("  \n[root@bozkurt] Hedef Bilgisi:\n | URL: "+boldwhite+"%s"+reset+"\n | IP: "+boldwhite+"%s"+reset+"\n  ")%(data,ip)
			print("[root@bozkurt] Herhangi Bir Cloudflare Erisimi Engellidigine Dair Kontrol Ediliyor ...")
			cloudflare()
			redirectcheck()
		except(IOError):
			print("[root@bozkurt]"+red+"Siteye Baglanirken Hata Olustu! Site Kapali Olabilir."+reset)
			again()
	def responseheadercheck():
		...
		for i in page.headers:
			if i.lower() in str(headers).lower():
				...
				headersfound.append(structure)
		if cj == 0:
			caution.append("[root@bozkurt]"+red+"X-Frame-Options Basligi Eksik\n"+reset+"[!] "+red+"Sayfa Savunmasiz Olabilir"+boldred+"Click Jacking\n"+reset+"[!] "+boldred+page.geturl()+reset+"\n[root@bozkurt] ClickJacking hakkinda:: [ "+green+"https://www.owasp.org/index.php/Clickjacking"+reset+" ]")
		print("[root@bozkurt] Ilginc Basliklar Bulundu::")
		for i in headersfound:
			print(" | %s")%(i)
		if len(interesting) != 0:
			print("\n[root@bozkurt] Basliklardan Bilgi Toplaniyor:")
	...
	def parameterarrange(payload):
		...
		for n in parameters:
			try:
				print("'%s' Parametresi Kontrol Ediliyor")%n[0]
				...
				pass
	def SQLIscan(site):
		print("[root@bozkurt] [SQLI] Tarayici Baslatildi ...\n")
		vuln = []
		payloads = {
					'2':'"',
					'1':'\''
		...
		errors = {
					'MySQL ':' SQL soz diziminizde bir hata var;;',
					'Oracle ':' SQL komutu uygun sekilde sona ermedi',
					'MSSQL ':' Karakter dizgisinden sonraki kapatilmamis tirnak isareti',
					'PostgreSQL ':' soz dizimi hatasi ya da yakininda'
		}
		...
		for n in parameters:
				...
				for i in payloads:	
					...
					for i in errors:
						if errors[i] in source:#htmlcode[0].contents[0]:
							dbfound = " | Back-End Veritabani: "+green+str(i)+reset
							found = 1
							break
					if found != 1:
						break
			...
			if found == 1:
				print("[root@bozkurt]"+red+"SQL Injection Guvenlik Acigi Bulundu!"+reset)
				print dbfound
				vuln.append("'"+n[0]+"'")
				found = 0
		if len(vuln) != 0:
			print(" | Hassas Parametreler:"), 
			for i in vuln:
				print(i),

		else:
			print("[root@bozkurt] Hassas degil")
	def XSSscan(site):
		print("[root@bozkurt] [ XSS ] Tarayici Baslatiliyor ...")
		vuln = []
		payloads = {
					'3':'d4rk();"\'\\/}{d4rk',
		...
		for n in parameters:
			...
			if found == 1:
				...
				found = 0
		if len(vuln) != 0:
			print("[root@bozkurt]"+red+"Hassas Parametreler:"+reset), 
			for i in vuln:
				print(boldred+i+reset),
		else:
			print("[root@bozkurt] Hassas degil")
	def portscanner():
		print("[i] Soz Dizimi	:	Fonksiyon")
  		print("    23,80,120	:	Belirli Baglanti Noktalarini Tarama, ornegin, Tarama Portu 23,80 ve 120")
  		print("    23-80	:	Cesitli Portlari Tarar, orn. 23 - 80 Portlarini Tarama")
  		print("    23   	:	Tek bir portu tarar, orn. Tarama 23")
  		print("    all  	:	20 ile 5000 arasindaki tum baglanti noktalarini tarar")
  		print(" ")
  		portoption = raw_input("[root@bozkurt] Aralik Veya Baglani Noktasini Girin:\n    > ")
  		wasmultiple		 = 		0
  		wasrange		 = 		0
  		wasone			 =		0
  		if ',' in portoption:
  			...
  			for i in multipleport:
  				if not str(i).isdigit():
  					print("[root@bozkurt] Yanlis Soz Dizimi!")
  					notexpected = 1
  			if notexpected == 1:
  				again()
  			...
  			print("[root@bozkurt] Yanlis Soz Dizimi!")
  			again()
		print("[root@bozkurt] Tarama% s Hedefe Port / s:% s")%(len(totallength),ip)
		ports = 5000
		found = 1
		protocolname = 'tcp'
		progress = 20
	...
	def enumform(listofIDs,listofnames):
		...
		if lengthofnames < 12:
			lengthofnames = 12
		print "[i] "+green+"Asagidaki kullanici adi bulundu:"+reset
	...
	def wpbackupscanner():
		...
		print("[+] Wordpress Yedeklemeleri Araniyor...")
		print("[?] Atlamak icin CTRL+C tuslarina basiniz ")
		progress = 0
		backup = []
		backupurl = []
		try:
			for i in backups:
				progress += 1
				sys.stdout.write("\r[+] Isleniyor %i / %s ..."% (progress,len(backups)))
				sys.stdout.flush()
				sock(i)
				if page.getcode() == 200:
					detecting = sock(i,"1")
	...
	def wpenumeration():
		...
		url = page.geturl()
		if page.geturl() != host:
			print("[i] Uzak Ana Bilgisayar Yonlendiriliyor '"+str(url)+"' \n    Yonlendirmenin Ardindan ")
			host = page.geturl()
		print("\n[+] Tarama Baslatildi : "+lightgreen+"%s"+reset) % time.strftime("%c")
		print "[+] Kullanicilar Sayiliyor..."
		T = 33
		found = 0
		listofusernames = []
		listofids = []
		...
		if found == 0:
			print("[+] "+red+"Kullanici Adlari Bulunamadi"+reset)
		else:
			enumform(listofids,listofusernames)
		print("[+] Numaralandirma Tamamlandi.")
		print("[+] Tarama Sonladi : "+lightgreen+"%s"+reset) % time.strftime("%c")
	def wpscanner():
		print("  \n[+] Wordpress Taraniyor")
		wp = 0
		i = 'wp-admin/'
	...
	def redirectcheck():
		...
		redirect = 0
		print("[+] Yonlendirme Kontrol Ediliyor")
		page = urllib.urlopen(host)
		url = page.geturl()
		if page.geturl() != host:
			option = raw_input("[i] "+boldgrey+"Ana Bilgisayar Yonlendiriliyor "+str(url)+reset+" \n    Varsayilan host olarak ayarlanilsin mi? [Y/N]:\n    > ")
			if option.lower() == "y":
				host = page.geturl()
				redirect = 1
		else:
	...
	def again():
		...
		sqliscanner		=		"off"
		# -- Swiches Reset --
		inp = raw_input("\n[+] [C]ikis yap yada [T]ekrar Baslat? (e/a)").lower()
		if inp == 'a':
			bozkurt()
		elif inp == 'e':
			exit()
...
# -- Program Structure Start -- 
	...
	try:
		global host
		host = raw_input("[+] Domain Ismini Giriniz \n    orn, site.com\n    > ")
		if 'https://' in host:
			pass
		elif 'http://' in host:
			pass
